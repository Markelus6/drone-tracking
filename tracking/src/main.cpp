...
// nanotrack_fc — NanoTrack NCNN on orchestrator SHM → UDP bbox + MJPEG :5003.
// Standalone module — init via UDP :12347 {"cmd":"init","bbox_norm":[cx,cy,w,h]}.
// Performance: persistent mmap, frame stride, async JPEG, no frame.clone().
...
// ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р Persistent SHM Reader (mmap once, read many) ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р
...
static bool shm_open_persistent(const std::string& shm_name) {
    ...
    std::fprintf(stderr, "[NanoTrack] SHM persistent mmap OK: %s (%zu KB)\n", shm_name.c_str(), g_shm_size / 1024);
    return true;
}
...
// ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р MJPEG :5003 (async JPEG encode) ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р
...
static void mjpeg_accept_thread() {
    ...
    if (bind(srv, reinterpret_cast<struct sockaddr*>(&a), sizeof(a)) < 0) {
        std::fprintf(stderr, "[NanoTrack] MJPEG bind :%d failed\n", g_mjpeg_port);
        close(srv);
        return;
    }
    listen(srv, 4);
    std::printf("[NanoTrack] MJPEG :%d ready\n", g_mjpeg_port);
    while (g_running) {
...
// Double-buffered JPEG encode (off main thread)
...
// ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р Tracking overlay drawing ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р
...
static void draw_tracking(cv::Mat& img, float cx, float cy, float h, float score) {
    if (img.empty() || h <= 0.01f) return;
    ...
    cv::line(img, cv::Point(bb.x, bb.y), cv::Point(bb.x + tl, bb.y), magenta, 2, cv::LINE_AA);
    cv::line(img, cv::Point(bb.x, bb.y), cv::Point(bb.x, bb.y + tl), magenta, 2, cv::LINE_AA);
    cv::line(img, cv::Point(bb.x + bb.width, bb.y), cv::Point(bb.x + bb.width - tl, bb.y), magenta, 2, cv::LINE_AA);
    cv::line(img, cv::Point(bb.x + bb.width, bb.y), cv::Point(bb.x + bb.width, bb.y + tl), magenta, 2, cv::LINE_AA);
    char lbl[64];
    snprintf(lbl, sizeof(lbl), "NanoTrack %.2f", score);
    ...
    cv::drawMarker(img, cv::Point(cxp, cyp), magenta, cv::MARKER_CROSS, 20, 2, cv::LINE_AA);
    cv::circle(img, cv::Point(cxp, cyp), 5, green, -1, cv::LINE_AA);
}

// ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р CMD thread ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р
...
static void cmd_thread(int port) {
    ...
    if (bind(sock, reinterpret_cast<struct sockaddr*>(&a), sizeof(a)) < 0) {
        std::fprintf(stderr, "[NanoTrack] cmd bind :%d failed\n", port);
        close(sock);
        return;
    }
    std::printf("[NanoTrack] Commands UDP :%d (init/reset)\n", port);
    char buf[512];
    while (g_running) {
...
// ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р MAIN ╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р╤В╨д╨Р


// ─── Manual Capture Web UI (:5004) ──────────────────────────────────────────
static void manual_capture_thread() {
    int port = 5004;
    int srv = socket(AF_INET, SOCK_STREAM, 0);
    if (srv < 0) { fprintf(stderr, "[ManualCapture] socket fail\n"); return; }
    int opt = 1;
    setsockopt(srv, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    struct sockaddr_in a{};
    a.sin_family = AF_INET;
    ...
    if (bind(srv, (struct sockaddr*)&a, sizeof(a)) < 0) {
        fprintf(stderr, "[ManualCapture] bind :%d fail\n", port);
        close(srv);
        return;
    }
    listen(srv, 4);
    fprintf(stderr, "[ManualCapture] Web UI :%d ready\n", port);

    const char* page = R"rawliteral(
<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>NanoTrack Capture</title>
<style>
...
<div id="wrap">
  <img id="video" src="http://HOST:5003/" onerror="this.src='data:,'">
...
cvs.addEventListener('mouseup', e => {
    ...
    while (g_running) {
        ...
        if (strstr(req, "GET /api/capture?")) {
            ...
            fprintf(stderr, "[ManualCapture] cx=%.3f cy=%.3f w=%.3f h=%.3f\n", cx, cy, w, h);

            char reply[256];
            int rlen = std::snprintf(reply, sizeof(reply), "Captured! cx=%.3f cy=%.3f w=%.3f h=%.3f", cx, cy, w, h);
            char resp[512];
            int nresp = std::snprintf(resp, sizeof(resp),
                "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nConnection: close\r\n\r\n%s", reply);
            send(cfd, resp, nresp, 0);
            close(cfd);
            continue;
        }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--npu-core" && i + 1 < argc) npu_core = std::atoi(argv[++i]);
        else if (a == "--viz-port" && i + 1 < argc) g_mjpeg_port = std::atoi(argv[++i]);
    }
    if (const char* e = std::getenv("NANOTRACK_MODELS_DIR")) model_dir = e;
    if (const char* e = std::getenv("NANOTRACK_CMD_PORT")) cmd_port = std::atoi(e);
    ...
    if (!tracker.load_models(model_dir)) {
        std::fprintf(stderr, "[NanoTrack] model load failed from %s\n", model_dir.c_str());
        return 1;
    }
    std::printf("[NanoTrack] Models OK: %s  cam=%s  telem=:%d  cmd=:%d  viz=:%d  stride=%d\n",
    ...
    if (!shm_open_persistent(shm)) {
        std::fprintf(stderr, "[NanoTrack] SHM failed %s — start orch_daemon first\n", shm.c_str());
        return 1;
    }

    cv::Mat probe;
    if (!shm_read(probe)) {
        std::fprintf(stderr, "[NanoTrack] SHM empty %s\n", shm.c_str());
    ...
    float smooth_cx = 0.f, smooth_cy = 0.f, smooth_h = 0.f, smooth_score = 0.f;
    ...
    // Velocity prediction: last 5 positions
    ...
    float cur_speed = 0;
    // Size freeze: stop size update after stable frames
    int stable_frames = 0;
    float frozen_h = 0;
    bool size_frozen = false;
    // Re-acquisition: after loss, predict and retry
    float reacq_cx = 0, reacq_cy = 0, reacq_h = 0;
    int reacq_attempts = 0;
    static const int MAX_REACQ = 8;
    float anchor_cx = 0.f, anchor_cy = 0.f, anchor_h = 0.f;
    ...
    while (g_running) {
        ...
        // Frame stride: skip N-1 frames between tracker updates
        bool should_track = (frame_id % frame_stride == 0);

        // Reset must be consumed before a pending init. The GCS deliberately
        // sends reset then init; processing these in the opposite order erased
        // a freshly initialized template.
        if (g_need_full_reset.exchange(false)) {
            ...
            lost_streak = 0;
            smooth_init = false;
            smooth_cx = smooth_cy = smooth_h = smooth_score = 0;
            vel_count = 0;
            vel_idx = 0;
            ...
            cur_speed = 0;
            anchor_set = false;
            fprintf(stderr, "[NanoTrack] FULL RESET\n");
        }

        if (g_need_init.exchange(false)) {
            const int w = frame.cols, h = frame.rows;
            const float nh = std::max(0.06f, std::min(0.45f, g_init_h));
            const float nw = std::max(0.06f, std::min(0.45f, g_init_w > 0.01f ? g_init_w : nh));
        ...
        if (!g_tracking && reacq_h > 0.01f && should_track) {
            if (reacq_cx > 0.001f && reacq_cy > 0.001f) {
                ...
                std::snprintf(hold, sizeof(hold),
                    ...
                    "\"class_id\":0,\"conf\":%.3f}",
                    cam_name.c_str(), reacq_cx, reacq_cy, reacq_h, 0.15f);
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_hb).count() >= 1) {
            ...
            fprintf(stderr,
                    "[NanoTrack] FPS=%.1f track_ms=%.1f/%.1f frames=%d tracking=%d "
            ...
            stat_track_ms_sum = 0;
            stat_track_ms_max = 0;
            const char* hb = "{\"tracker\":\"nanotrack\",\"alive\":true}";
    ...
    shm_close_persistent();
    close(telem_sock);
    std::printf("[NanoTrack] stopped (%d frames)\n", frame_id);
    return 0;
}
