#!/usr/bin/env bash
# Start / stop / status for drone-tracking + Betaflight MSP on Orange Pi / RK3588.
set -euo pipefail
...
Environment:
  ...
  TRACKER            nano | nanov3 | light | csrt | kcf | mosse | mil | medianflow | tld
                     ...
                     (also stubs: tctrack | mixformer | ostrack | cftrack)
                     default: light
  TRACK_CAMERA       V4L2 device   (default: /dev/cam_usb2)
  VISION_TELEMETRY_PORT  UDP telem (default: 12345) — owned by msp_betaflight
...
do_status() {
  ...
  for pat in orch_daemon nanotrack_fc lighttrack_fc multitrack_fc msp_betaflight.py stats_web.py; do
    if _is_running "$pat"; then
  ...
  if command -v ss >/dev/null 2>&1; then
    ss -lntup 2>/dev/null | grep -E ':(500[3-9]|501[0-9]|502[0-3]|8090|12345|1234[79]|1235[13579]|1236[135])\b' || true
  fi
  if command -v curl >/dev/null 2>&1; then
